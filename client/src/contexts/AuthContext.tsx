...
import { findExistingPushSubscription } from "@/lib/push";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const unsubscribePushMutation = trpc.push.unsubscribe.useMutation();
  ...
  const logout = useCallback(async () => {
    // Desvincula este aparelho da conta atual sem cancelar a subscription
    // local. A próxima conta poderá reutilizar e reassociar o mesmo endpoint.
    try {
      const subscription = await findExistingPushSubscription();
      if (subscription) {
        await unsubscribePushMutation.mutateAsync({ endpoint: subscription.endpoint });
      }
    } catch {
      // Logout não pode ficar bloqueado por falha de rede/push.
    }
    ...
    try {
      ...
      window.dispatchEvent(new Event("notifique-me:auth-session-ended"));
  ...
  }, [logoutMutation, unsubscribePushMutation, utils.auth.me]);
