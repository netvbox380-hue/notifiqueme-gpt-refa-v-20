...
export function DeliveryLogs({ notificationId }: DeliveryLogsProps) {
  const { data: deliveryData, isLoading } = trpc.notifications.deliveryMap.useQuery({ notificationId });

  if (isLoading) return <div className="p-4 animate-pulse">Carregando logs de entrega...</div>;
  if (!deliveryData || deliveryData.items.length === 0) return <div className="p-4 text-muted-foreground">Nenhum log de entrega encontrado.</div>;
  ...
  return (
    <div className="border-2 border-border rounded-lg overflow-hidden">
      <Table>
        ...
        <TableBody>
          {deliveryData.items.map((log) => (
            <TableRow key={log.deliveryId} className="hover:bg-secondary/50">
              <TableCell className="font-medium">{log.name || "Usuário Desconhecido"}</TableCell>
              <TableCell>
