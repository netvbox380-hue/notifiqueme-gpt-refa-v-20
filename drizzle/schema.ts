...
export const pushSubscriptions = pgTable(
  ...
  (t) => ({
    ...
    idxPushSubsTenantUser: index("idx_push_subscriptions_tenant_user").on(t.tenantId, t.userId),
  })
);
...
/**
 * Capturas de diagnóstico sob demanda.
 * O admin do tenant inicia/encerra; os eventos expiram sete dias após o encerramento.
 */
export const diagnosticCaptures = pgTable(
  ...
  (t) => ({
    idxDiagnosticCapturesTenantUser: index("idx_diagnostic_captures_tenant_user").on(t.tenantId, t.userId),
    idxDiagnosticCapturesExpires: index("idx_diagnostic_captures_expires").on(t.expiresAt),
  }),
);

export const diagnosticCaptureEvents = pgTable(
  "diagnostic_capture_events",
  {
    id: serial("id").primaryKey(),
    captureId: integer("captureId")
      .notNull()
      .references(() => diagnosticCaptures.id, { onDelete: "cascade" }),
    tenantId: integer("tenantId")
      .notNull()
      .references(() => tenants.id, { onDelete: "cascade" }),
    userId: integer("userId")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    installationId: varchar("installationId", { length: 100 }).notNull(),
    clientEventId: varchar("clientEventId", { length: 100 }).notNull(),
  ...
  (t) => ({
    idxDiagnosticEventsCapture: index("idx_diagnostic_events_capture").on(t.captureId, t.eventAt),
    uniqDiagnosticClientEvent: uniqueIndex("uniq_diagnostic_client_event").on(t.captureId, t.installationId, t.clientEventId),
  }),
);