...
CREATE INDEX IF NOT EXISTS idx_push_subscriptions_tenant_user ON push_subscriptions("tenantId", "userId");
...
-- ============================
-- Captura de diagnóstico sob demanda (retenção de 7 dias)
-- ============================
CREATE TABLE IF NOT EXISTS diagnostic_captures (
  id SERIAL PRIMARY KEY,
  "tenantId" INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  "userId" INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  "requestedByUserId" INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS diagnostic_capture_events (
  id SERIAL PRIMARY KEY,
  "captureId" INTEGER NOT NULL REFERENCES diagnostic_captures(id) ON DELETE CASCADE,
  "tenantId" INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  "userId" INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_diagnostic_captures_tenant_user
  ON diagnostic_captures("tenantId", "userId");
CREATE INDEX IF NOT EXISTS idx_diagnostic_captures_expires
  ON diagnostic_captures("expiresAt");
CREATE INDEX IF NOT EXISTS idx_diagnostic_events_capture
  ON diagnostic_capture_events("captureId", "eventAt");
CREATE UNIQUE INDEX IF NOT EXISTS uniq_diagnostic_client_event
  ON diagnostic_capture_events("captureId", "installationId", "clientEventId");