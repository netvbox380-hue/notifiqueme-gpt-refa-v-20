...
export const superAdminRouter = router({
  ...
  resetAdminPassword: ownerProcedure
    .input(z.object({ id: z.number(), password: z.string().min(4).max(128) }))
    .mutation(async ({ input }) => {
      const db = await getDb();
      const target = await db.select({ id: users.id, role: users.role }).from(users).where(eq(users.id, input.id)).limit(1);
      if (!target[0] || !["admin", "reseller"].includes(target[0].role)) {
        throw new TRPCError({ code: "NOT_FOUND", message: "Administrador não encontrado" });
      }
      await db.update(users).set({ passwordHash: await hashPassword(input.password), updatedAt: new Date() }).where(eq(users.id, input.id));
      return { success: true };
    }),
