...
import { assertAttachmentPolicy, assertAudiencePolicy, normalizePositiveIds } from "../_core/notificationPolicy";
...
async function resolveNotificationRecipientIds(
  ...
  const recipientRole = audience === "admins" ? "admin" : "user";

  assertAudiencePolicy({ actorRole: actor?.role, audience, targetType });

  if (targetType === "all") {
    const rows = await db.select({ id: users.id }).from(users).where(
      audience === "admins"
        ? and(eq(users.tenantId, tenantId), eq(users.role, recipientRole))
        : isOwner
          ? and(eq(users.tenantId, tenantId), eq(users.role, recipientRole))
          : and(eq(users.tenantId, tenantId), eq(users.role, recipientRole), eq(users.createdByAdminId, actorId)),
    );
    return Array.from(new Set(rows.map((r: any) => Number(r.id)).filter(Number.isFinite)));
  }

  const normalizedTargetIds = normalizePositiveIds(targetIds ?? []);
  if (!normalizedTargetIds.length) return [];

  if (targetType === "users") {
    const rows = await db.select({ id: users.id }).from(users).where(
      audience === "admins"
        ? and(eq(users.tenantId, tenantId), eq(users.role, recipientRole), inArray(users.id, normalizedTargetIds))
        : isOwner
          ? and(eq(users.tenantId, tenantId), eq(users.role, recipientRole), inArray(users.id, normalizedTargetIds))
          : and(eq(users.tenantId, tenantId), eq(users.role, recipientRole), eq(users.createdByAdminId, actorId), inArray(users.id, normalizedTargetIds)),
    );
    return Array.from(new Set(rows.map((r: any) => Number(r.id)).filter(Number.isFinite)));
  }
  ...
  const members = await db
    ...
    .where(and(
      ...
      eq(users.role, "user"),
      ...(isOwner ? [] : [eq(groups.createdByAdminId, actorId), eq(users.createdByAdminId, actorId)]),
    ));

  return Array.from(new Set(members.map((m: any) => Number(m.userId)).filter(Number.isFinite)));
}

function notificationScopeClause(ctx: any) {
  if (ctx.user?.role === "owner") return eq(notifications.createdBy, Number(ctx.user.id));
  return and(
    eq(notifications.tenantId, requireTenant(ctx)),
    eq(notifications.createdBy, Number(ctx.user.id)),
  );
}
...
export const notificationsRouter = router({
    ...
    .query(async ({ ctx, input }) => {
      ...
      const allowedNotificationIds = notificationScopeRows
        .filter((row: any) => {
          ...
          return Number(row.tenantId) === Number(ctx.user?.tenantId ?? 0) && Number(row.createdBy) === Number(ctx.user.id);
        })
  ...
  deliveryMap: adminOnlyProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (Number(notification.createdBy) !== Number(ctx.user.id)) {
        throw new TRPCError({ code: "FORBIDDEN", message: "Mensagem criada por outro emissor" });
      }
  ...
  feedbackSummary: adminOnlyProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const scoped = await db
        .select({ id: notifications.id })
        .from(notifications)
        .where(and(inArray(notifications.id, input.notificationIds), notificationScopeClause(ctx)));
      const allowedIds = scoped.map((row: any) => Number(row.id));
      if (!allowedIds.length) return { byId: {} };
      ...
      const rows = await db
        ...
        .from(deliveries)
        .where(and(inArray(deliveries.notificationId, allowedIds), isNotNull(deliveries.feedback)))
  ...
  send: adminOnlyProcedure
    .input(
      z.object({
        ...
        audience: z.enum(["users", "admins"]).optional(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const audience = isOwner ? (input.audience ?? "admins") : "users";
      ...
      let validatedAttachmentIds: number[] = [];
      if (allAttachmentKeys.length) {
        ...
        for (const file of fmeta) {
          await assertTenantInScopeOrThrow(db, ctx.user, Number(file.tenantId));
          assertAttachmentPolicy({
            actorRole: ctx.user?.role,
            actorId,
            messageTenantId: tenantId,
            fileTenantId: Number(file.tenantId),
            uploadedBy: Number(file.uploadedBy),
            relatedNotificationId: file.relatedNotificationId,
          });
        }

        const totalBytes = fmeta.reduce((acc, file) => acc + Number(file.fileSize ?? 0), 0);
        const imagesCount = fmeta.filter((file) => String(file.mimeType || "").startsWith("image/")).length;
        const videosCount = fmeta.filter((file) => String(file.mimeType || "").startsWith("video/")).length;
        ...
        validatedAttachmentIds = fmeta.map((file) => Number(file.id));
      }

      const now = new Date();

      // Operação atômica: créditos, notificação, anexos e deliveries são
      // confirmados juntos. Qualquer falha reverte todo o envio no banco.
      const notificationId = await db.transaction(async (tx) => {
        ...
        if (validatedAttachmentIds.length) {
          const linkedFiles = await tx
            .update(files)
            .set({ relatedNotificationId: createdId })
            .where(
              and(
                eq(files.tenantId, tenantId),
                inArray(files.id, validatedAttachmentIds),
                isNull(files.relatedNotificationId),
              ),
            )
            .returning({ id: files.id });

          if (linkedFiles.length !== validatedAttachmentIds.length) {
            throw new TRPCError({
              code: "CONFLICT",
              message: "Um dos anexos foi utilizado por outro envio. Atualize a página e tente novamente.",
            });
          }
        }
      ...
      // Push é externo ao banco e só roda depois do commit. A inbox já está
      // persistida mesmo quando o dispositivo não possui subscription.
      ...
      // Atualiza somente o estado do canal push. Falha nessa auditoria não
      // desfaz a mensagem já confirmada e disponível na inbox.
      try {
        if (pushResult.sentUserIds.length) {
          await db
            .update(deliveries)
            .set({ status: "delivered", errorMessage: null } as any)
            .where(
              and(
                eq(deliveries.tenantId, tenantId),
                eq(deliveries.notificationId, notificationId),
                inArray(deliveries.userId, pushResult.sentUserIds),
              ),
            );
        }

        if (pushResult.skippedUserIds.length) {
          await db
            .update(deliveries)
            .set({
              status: "sent",
              errorMessage: "Sem subscription push ativa; mensagem disponível na caixa de entrada",
            } as any)
            .where(
              and(
                eq(deliveries.tenantId, tenantId),
                eq(deliveries.notificationId, notificationId),
                inArray(deliveries.userId, pushResult.skippedUserIds),
              ),
            );
        }

        for (const userId of pushResult.failedUserIds) {
          await db
            .update(deliveries)
            .set({
              status: "failed",
              errorMessage:
                pushResult.errorsByUserId[userId] ||
                "Falha no push; mensagem disponível na caixa de entrada",
            } as any)
            .where(
              and(
                eq(deliveries.tenantId, tenantId),
        ...
        console.error("[PUSH] falha ao registrar status das deliveries", {
          tenantId,
          notificationId,
          error: statusError,
        });
      ...
      console.info("[PUSH] despacho concluído", {
        tenantId,
        notificationId,
        recipients: userIds.length,
        sent: pushResult.sentUserIds.length,
        failed: pushResult.failedUserIds.length,
        skipped: pushResult.skippedUserIds.length,
        pushConfigured: pushResult.pushConfigured,
        errorsByUserId: pushResult.errorsByUserId,
      });
      ...
      return {
        ...
        push: {
          configured: pushResult.pushConfigured,
          sent: pushResult.sentUserIds.length,
          failed: pushResult.failedUserIds.length,
          skipped: pushResult.skippedUserIds.length,
        },
