...
async function handleJob(type: JobType, payloadStr: string) {
  ...
  if (type === "dispatch_notification") {
    ...
    const pushSentUserIds = Array.from(new Set(pushResult?.sentUserIds ?? []));
    const pushFailedUserIds = Array.from(new Set(pushResult?.failedUserIds ?? []));
    const pushSkippedUserIds = Array.from(new Set(pushResult?.skippedUserIds ?? []));
