...
export async function getTenantPlanLimits(tenantId: number, executor?: DbExecutor) {
...
export async function getCreditsUsageToday(tenantId: number, executor?: DbExecutor) {
  const db = executor ?? await getDb();
  if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB indisponível" });
...
export async function checkAndConsumeCredits(params: {
  ...
  const { tenantId, cost, executor } = params;
  ...
  const { limits, plan } = await getTenantPlanLimits(tenantId, db);
  const day = utcDay();
  ...
  if (!Number.isFinite(newUsed)) {
    ...
    const cur = await getCreditsUsageToday(tenantId, db);
