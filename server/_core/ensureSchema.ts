...
export async function ensureSchema(db: PostgresJsDatabase<any>) {
  ...
  `);
  await db.execute(sql`
    UPDATE push_subscriptions ps SET "tenantId" = u."tenantId"
    FROM users u WHERE ps."userId" = u.id AND ps."tenantId" IS NULL;
  `);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_push_subscriptions_tenant_user ON push_subscriptions("tenantId", "userId");`);
